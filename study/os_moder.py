#==============OS模块=================
"""
os模块主要实现了当前操作系统所获取相关环境数据的接口模式，可实现当前操作系统的文件操作（增删改查）、路径获取、环境数据等；
    注意：使用os模块进行调用方法时，一定要注意当前方法所使用的平台
"""
#==============文件的权限操作=================
#更改文件的访问权限
os.chmod(r"g:/GIT",mode=777)

#获取当前的环境变量，返回的是字典，可以通过环境变量名
